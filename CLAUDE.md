...
- Branch: `phase-5/consumption-history` — Created ConsumptionAnalyticsService (weekly/monthly grouping, averages, ground comparison), WeeklyConsumption/MonthlyConsumption/UnitConsumption models, analytics providers, ConsumptionHistoryScreen (summary tiles, fl_chart bar/line chart with threshold line, history list), GroundConsumptionScreen (per-unit comparison bar chart, date range picker), updated ElectricityOverviewScreen (avg weekly + trend indicator per unit card, View Ground Consumption button), GoRouter routes, 750 tests passing ⏳ in progress
