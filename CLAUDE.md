...
- Branch: `phase-4/rent-payment-flow` — Created RentPaymentScreen (pre-filled amount, quick amount buttons Full/Half/Custom, payment method dropdown, auto-today date, one-tap confirm), QuickAmountButtons widget, updated RentListScreen with payment navigation and paid details bottom sheet, wired QuickAddFab rent action to /rent, GoRouter /rent/pay route, HmsDropdown isExpanded fix, 18 tests (unit + widget) ✅ merged
- Branch: `phase-4/rent-history` — Created RentHistoryService (per-tenant queries, totals, counts), rent history providers, TenantRentHistoryScreen (summary tiles, year-grouped timeline, payment cards with status and detail bottom sheet), linked from TenantDetailScreen, GoRouter route, 20 tests (12 unit + 8 widget) ✅ merged
- Branch: `phase-4/rent-summary-dashboard` — Created RentSummaryService (expected/collected/outstanding/collection rate per current ground), summary providers reacting to ground selection, updated HealthScoreService with real rent score, updated MonthlyReportService with real rent data, RentIncomeLinkService auto-creates income on payment, dashboard rent summary tile, tests ✅ merged
- Branch: `phase-4/rent-alerts` — Replaced sample alerts with real overdue rent alerts (AlertGeneratorService async, RentSummaryService integration), RentNotificationService (overdue notifications, due reminders, cancellation on payment), wired into RentGenerationService, RentPaymentScreen, and main.dart startup, tests (667 passing) ✅ merged
- **Phase 4 — Rent Payment Tracking: COMPLETE** ✅

### Phase 5 — Electricity Monitoring System

- Branch: `phase-5/meter-registry` — Created ElectricityMeter Freezed model (hasThreshold getter), MeterService (registerMeter, updateMeter, replaceMeter with history preservation, getActiveMeter, streamActiveMeter, getAllMeters, updateThreshold), meter providers (meterServiceProvider, activeMeterProvider, allMetersProvider), MeterRegistrationScreen (create/edit mode), MeterReplacementScreen (shows current meter info + warning banner), ElectricityOverviewScreen (all units with meter status, Register button for unmapped units), updated UnitListScreen with electric meter icon and "no meter" subtitle hint, updated HomeScreen Drawer with Electricity nav item (routes to ground electricity overview or grounds list), GoRouter routes (/meter/register, /meter/edit, /meter/replace, /:groundId/electricity), barrel exports, 26 tests (3 model + 9 service + 14 widget) ✅ merged
- Branch: `phase-5/meter-reading-entry` — Created MeterReading Freezed model, MeterReadingService (auto-calculation of unitsConsumed, MeterResetRequiredException, meter reset handling, getTotalConsumption range queries), reading providers (meterReadingServiceProvider, latestReadingProvider, readingsProvider), RecordReadingScreen (live consumption indicator, meter reset confirmation dialog, date picker, notes), QuickReadingScreen (batch entry for all units with active meters, per-unit save + Save All, focus chain, live calc per card), updated ElectricityOverviewScreen with Quick Reading AppBar button, wired QuickAddFab Electricity action to quick-reading route, GoRouter routes (/quick-reading, /meter/:meterId/record), 711 tests passing ✅ merged
- Branch: `phase-5/tanesco-tariff-config` — Created TariffService (tier-by-tier cost calculation, defaults D1 rates, Super Admin write, Firestore round-trip with null↔infinity), tariff providers (tariffServiceProvider, currentTariffsProvider, calculateCostProvider), TariffConfigScreen (editable tier table with live preview, add/remove tiers, Super Admin guard), TariffTierEditor widget, updated RecordReadingScreen with estimated cost display, GoRouter /settings/tariffs route, TANESCO Settings drawer item for Super Admin, 731 tests passing ✅ merged
- Branch: `phase-5/consumption-alerts` — Created ConsumptionAlertService (threshold detection, severity classification, spike detection), ConsumptionWarning model, alert providers reacting to ground selection, updated AlertGeneratorService with real electricity alerts, ElectricityNotificationService for critical consumption notifications, wired into main.dart startup, ConsumptionWarningsScreen (/electricity/warnings route), 766 tests passing ✅ merged
...
- Branch: `phase-6/sms-parser` — Created ParsedSmsResult model, SmsParserService (regex-based extraction for English + Swahili DAWASCO SMS formats, amount/readings/due date/period detection, graceful partial parsing), wired into AddWaterBillScreen SMS tab (parse → pre-fill → review), comprehensive unit tests for English/Swahili/edge cases, 872 total tests passing ✅ merged
- Branch: `fix/water-bill-detail-issues` — Fixed WaterBillDetailScreen: typed AsyncValue&lt;WaterSurplusDeficit&gt; replacing dynamic, removed unsafe double casts, fixed async context gap in delete flow, hide contributions card when no tenants, 893 tests passing ✅ merged
- Branch: `phase-6/water-contributions` — Created WaterContributionService (setup/deactivate via RecurringTransactionEngine, mark paid, surplus/deficit calculation), WaterSurplusDeficit model, contribution providers, WaterContributionsScreen (monthly view, quick payment bottom sheet, surplus/deficit summary), WaterSettingsScreen (default amount config), wired into TenantService and MoveOutService, updated WaterBillsScreen and WaterBillDetailScreen with surplus/deficit, GoRouter routes, tests, 893 total tests passing ✅ merged
- Branch: `phase-6/water-alerts-dashboard` — Created WaterSummaryService (monthly cost, unpaid counts, due soon, surplus/deficit per ground), water summary providers (8 Riverpod providers reacting to currentGroundProvider), updated AlertGeneratorService with real water alerts (critical for overdue, warning for due soon, info for deficit), WaterNotificationService (due date reminders with deterministic IDs, overdue notifications, cancel on payment), wired into AddWaterBillScreen/WaterBillDetailScreen lifecycle and main.dart startup, updated HealthScoreService.buildScore to accept billsRate/billsActive, wired real bills score in health_score_provider, updated MonthlyReport model with waterBillTotal/waterContributionsCollected/waterSurplusDeficit fields, updated MonthlyReportService to fetch real water data, added Water section to MonthlyReportScreen, added _WaterSummaryTile to HomeScreen dashboard, updated barrel exports, 922 total tests passing ✅ merged
- **Phase 6 — Water Bill Tracking: COMPLETE** ✅

### Phase 7 — Financial Management

- Branch: `phase-7/income-tracking` — Created Income Freezed model (isRentIncome/hasGround getters), IncomeSource enum (label/value/icon, fromString), IncomeService (CRUD on top-level `incomes` collection, month/source/ground aggregation, auto-linked rent entries rejected on update/delete), rewired RentIncomeLinkService to build proper Income objects (isAutoLinked: true) via IncomeService, income providers (incomeService, allIncome, currentMonthIncome, currentMonthIncomeTotal reacting to currentGroundProvider, incomeBySource), IncomeListScreen (month navigation, source filter chips, green summary tiles, auto-linked lock indicator + read-only bottom sheet), AddIncomeScreen (manual entry with rent excluded from sources, optional ground association, edit mode), Finance drawer item, GoRouter routes (/finance/income/*), feature folder + barrel exports, 26 tests (model/enum + service auto-linked protection + list/add widget tests) ✅ merged
- Branch: `phase-7/expense-tracking` — Created Expense Freezed model (isUncategorized/hasGround getters), ExpenseCategory enum (14 categories with label/value/icon, fromString defaulting to uncategorized), ExpenseService (CRUD on top-level `expenses` collection with auto-linked protection on update/delete, category/ground aggregation, getExpensesByCategoryForPeriod, getTopCategories, getDailySpendingForMonth, createAutoLinkedExpense for module-sourced entries), expense providers (expenseService, allExpenses, currentMonthExpenses, currentMonthExpenseTotal/expensesByCategory/topExpenseCategories reacting to currentGroundProvider), ExpenseListScreen (month navigation, red summary tiles Total/Daily Average/Categories, dynamic category filter chips, auto-linked lock + read-only bottom sheet), AddExpenseScreen (speed-optimized: autofocus amount, category defaults to uncategorized, date defaults today, category-aware description hints, post-save Add Another/Done prompt, create+edit modes), CategoryBreakdownChart (fl_chart pie + tappable legend, reusable), wired QuickAddFab "Log an Expense" → /finance/expenses/add/uncategorized, Finance drawer item now → /finance/expenses, GoRouter routes (/finance/expenses, add, add/:category, :expenseId/edit), barrel exports, 21 new tests (model/enum + service + list/add/chart widget tests), 970 total tests passing ✅ merged
- Branch: `phase-7/budget-system` — Created Budget Freezed model (remainingAmount, spentPercentage clamped 0–999, isOnTrack/isNearLimit/isOverLimit at 80%/100% thresholds, statusLabel, compliance), BudgetService (set/get/stream by category+period with deterministic `{period}_{category}` doc IDs preserving spentAmount, recalculateSpent summing expenses per category, copyFromPreviousMonth skipping existing, setupRecurringBudgets via RecurringTransactionEngine, getTotalLimit/getTotalSpent/getWarningBudgets/getOverBudgets/getBudgetComplianceScore), budget providers (budgetService, budgetNotificationService, currentMonthBudgets, budgetForCategory, warning/over/complianceScore/totalLimit/totalSpent reacting to current period), BudgetScreen (month nav, Total Budget/Total Spent/Compliance summary tiles, colour-coded progress cards, post-frame recalc on open, empty state), BudgetProgressCard (AppCard + LinearProgressIndicator green<80/amber 80–99/red≥100, remaining/over text, static statusColor), BudgetSetupScreen (per-category HmsCurrencyfield prefilled from this/previous month, skips empty/zero, copy-forward checkbox → recurring configs), BudgetNotificationService (deterministic IDs, warning at 80% / exceeded at 100%, no double-notify when over), wired recalc + notify via ExpenseService onExpenseRecorded hook (covers all expense paths, lazy budget read avoids provider cycle), Budget drawer item, GoRouter routes (/finance/budget, /finance/budget/setup), barrel exports, 30 new tests (7 model + 11 service + 4 notification + 4 progress card + 3 screen + 2 setup), 1000 total tests passing ✅ merged
- Branch: `phase-7/financial-reports-dashboard` — Created FinancialSummary/GroundFinancialSummary/MonthlyTrend Freezed models (netPosition/isPositive/savingsRate getters), FinancialSummaryService (getNetPosition, getSummary with income/expense/budget aggregation, getPerGroundComparison, getMonthlyTrends, getCategoryTrends), financial summary providers (service, currentMonthSummary reacting to currentGroundProvider, perGroundComparison, monthlyTrends, netPosition), FinanceOverviewScreen (3-tab hub — Income/Expenses/Budget — with shared month selector, per-tab summary tiles + charts + recent 5 entries, pinned NetPositionCard below tabs), NetPositionCard (income/expenses/net lines, green/red tint), MonthlyTrendChart (fl_chart two-line income vs expenses), adapted CategoryBreakdownChart with optional labelOf/iconOf resolvers (reused for income sources), replaced ALL remaining mock data: HealthScoreService now wires real overdue (rent+water+budget, −10/item) and budget compliance scores with stockActive=false, MonthlyReportService now pulls real income/expense totals, top categories, aggregated overdue items, and per-ground comparison, MonthlyReportScreen gained a Financial Overview section (NetPositionCard + 6-month trend + expense breakdown), Dashboard gained a real Net Position tile and Finance drawer item now → /finance, GoRouter /finance route, barrel exports, 25 new tests (5 model + 5 service + 5 health/widget + finance overview 5 + net card 3 + trend chart 3 + report/dashboard section), 1028 total tests passing ✅ merged
